   ...
   switch (uMsg) {
      ...
      case WM_MOUSEMOVE: // Update the dragged item during the drag operation
         if (hDraggedItem) {
            ...
            ClientToScreen(hwnd, &pt);  // Now pt contains screen coordinates

            // Get the window's position on the screen (to adjust for borders/title bar)
            RECT windowRect;
            GetWindowRect(hwnd, &windowRect);  // Get window position relative to the screen

            // Adjust mouse position based on the window's position
            //pt.x -= windowRect.left;  // Adjust for the window's left position
            //pt.y -= windowRect.top;   // Adjust for the window's top position
